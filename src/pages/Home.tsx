...
/* ─── Section 6: CTA ─────────────────────────────────────────────────────── */
function Section6({ inView, onStart }: { inView: boolean; onStart: () => void }) {
  return (
    <div style={{ textAlign: "center", padding: "0 32px", position: "relative", zIndex: 2 }}>
      ...
      <motion.div initial={{ opacity: 0, y: 40 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.9, ease: "easeOut" }} style={{ position: "relative" }}>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.94 }} animate={inView ? { opacity: 1, scale: 1 } : {}} transition={{ delay: 0.5, duration: 0.7 }}>
          <motion.button onClick={onStart} style={{ display: "inline-flex", alignItems: "center", gap: 12, padding: "17px 46px", borderRadius: 100, background: `linear-gradient(135deg, ${C.lilac}, ${C.blush})`, border: "none", color: "white", fontFamily: "system-ui, sans-serif", fontWeight: 500, fontSize: "1.05rem", letterSpacing: "0.01em", cursor: "pointer", boxShadow: `0 8px 32px ${C.lilac}66`, position: "relative", overflow: "hidden" }}
...
export default function Home({ onStart }: { onStart: () => void }) {
  ...
  <div style={centered}><Section6 inView={s6v} onStart={onStart} /></div>
</div>
    </div>
  );
}
