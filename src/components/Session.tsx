...
function getOrbTheme(mode: AccentMode, soundType: Recommendation["soundType"]): OrbTheme {
  if (mode === "deep-spiral") return {
    bg: "bg-[#1a0a0a]", card: "bg-red-950/60", border: "border-red-800/40",
    label: "text-red-200", subtext: "text-red-300/80",
    button: "from-[#dc2626] to-[#b45309]",
    durationBtn: "border-red-800/40 text-red-200", durationHover: "hover:bg-red-900/40",
    orbGlow: "rgba(220, 38, 38, 0.5)", orbGlowOuter: "rgba(180, 83, 9, 0.2)",
    orbGradient: "radial-gradient(circle at 40% 35%, #7f1d1d, #dc2626 55%, #b45309)",
    breatheDuration: "7s",
  };
...
function BodyDoublingBadge({ theme }: { theme: OrbTheme }) {
  ...
  return (
    <div className="fixed bottom-5 left-0 right-0 flex justify-center pointer-events-none" style={{ zIndex: 10 }}>
      <span className={`text-xs ${theme.subtext} opacity-60 tracking-wide`}>{line}</span>
    </div>
  );
}
...
export default function Session({
  ...
  // Speak recommendation aloud when the pre-session card first appears
  useEffect(() => {
    const timer = setTimeout(() => speakRecommendation(rec), 600);
    return () => clearTimeout(timer);
  }, []);

  // Poll for analyser node once audio starts
  useEffect(() => {
    if (!running) return;
  ...
  if (running && duration) {
    return (
      ...
      >
        {analyser && <SoundVisualizer analyser={analyser} soundType={rec.soundType} />}
  ...
  return (
    <div className={`min-h-screen flex items-center justify-center ${theme.bg} px-6 accent-transition`}>
      <div
        className={`max-w-md w-full ${theme.card} rounded-3xl p-7 shadow-lg border ${theme.border}`}
      ...
      >
        ...
        <p className={`text-xs font-medium ${theme.label} uppercase tracking-wide mb-2`}>Pick a duration</p>
